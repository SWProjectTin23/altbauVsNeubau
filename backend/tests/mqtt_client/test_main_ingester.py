...
def test_on_connect_failure_logs_error(mocker):
    mock_client = MagicMock()
    mock_logger = mocker.patch("mqtt_client.main_ingester.logger")

    on_connect(mock_client, {}, {}, 1)

    mock_logger.error.assert_called_once_with("MQTT connection failed. Code=%s", 1)
...
def test_connect_db_failure_logs_error(mocker):
    mock_logger = mocker.patch("mqtt_client.main_ingester.logger")
